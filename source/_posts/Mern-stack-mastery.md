...
Apart from the query language used, automatic ***indexing*** in MongoDB makes data retrieval fast and less painful. Each field stored has a unique **'_id'** and other fields that can be removed or added as needed.
...
Express.js simplifies the process of ***error handling*** using middlewares that catch and process errors during the request-response cycle.
...
With its routing capabilities, Express is commonly used to create ***RESTful APIs***. It makes the process of defining routes that map into different CRUD operations ( Create, Read, Updata and DELETE ) on API endpoints easy.
...
When the state of a component change, react re-renders that component and its children. This allows the component to have a local state where data can change in real time. This is ***state management*** and it is the core to creation of dynamic and interactive UI.
